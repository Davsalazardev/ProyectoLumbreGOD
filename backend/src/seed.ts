...
import { metricsAdvancedService } from './services/metricsAdvanced.service';
...
async function seed() {
  ...
  await runAnalysis(jsAnalysis.id, sampleJavaScriptCode, 'src/checkout.js');
  // Generate advanced metrics
  await metricsAdvancedService.assignSecurityMappings(jsAnalysis.id);
  await metricsAdvancedService.generateTestResults(jsAnalysis.id);
  await metricsAdvancedService.generateFileComplexity(jsAnalysis.id);
  await metricsAdvancedService.createSecurityHotspots(jsAnalysis.id);
  const jsMetrics = await prisma.metric.findFirst({ where: { analysisId: jsAnalysis.id } });
  if (jsMetrics) {
    await metricsAdvancedService.updateMetricsWithAdvanced(jsAnalysis.id);
    await metricsAdvancedService.recordMetricsEvolution(jsMetrics.id);
  }
  await metricsAdvancedService.recordQualityGateHistory(jsProject.id, 'FAILED', 'Too many critical issues');
  ...
  await runAnalysis(pyAnalysis.id, samplePythonCode, 'api/user_service.py');
  await metricsAdvancedService.assignSecurityMappings(pyAnalysis.id);
  await metricsAdvancedService.generateTestResults(pyAnalysis.id);
  await metricsAdvancedService.generateFileComplexity(pyAnalysis.id);
  await metricsAdvancedService.createSecurityHotspots(pyAnalysis.id);
  const pyMetrics = await prisma.metric.findFirst({ where: { analysisId: pyAnalysis.id } });
  if (pyMetrics) {
    await metricsAdvancedService.updateMetricsWithAdvanced(pyAnalysis.id);
    await metricsAdvancedService.recordMetricsEvolution(pyMetrics.id);
  }
  await metricsAdvancedService.recordQualityGateHistory(pyProject.id, 'FAILED', 'Security vulnerabilities detected');
  ...
  await runAnalysis(javaAnalysis.id, sampleJavaCode, 'src/UserService.java');
  await metricsAdvancedService.assignSecurityMappings(javaAnalysis.id);
  await metricsAdvancedService.generateTestResults(javaAnalysis.id);
  await metricsAdvancedService.generateFileComplexity(javaAnalysis.id);
  await metricsAdvancedService.createSecurityHotspots(javaAnalysis.id);
  const javaMetrics = await prisma.metric.findFirst({ where: { analysisId: javaAnalysis.id } });
  if (javaMetrics) {
    await metricsAdvancedService.updateMetricsWithAdvanced(javaAnalysis.id);
    await metricsAdvancedService.recordMetricsEvolution(javaMetrics.id);
  }
  await metricsAdvancedService.recordQualityGateHistory(javaProject.id, 'FAILED', 'Code complexity too high');
...
export default router;
  ...
  await runAnalysis(cleanAnalysis.id, cleanCode, 'src/auth.ts');
  await metricsAdvancedService.assignSecurityMappings(cleanAnalysis.id);
  await metricsAdvancedService.generateTestResults(cleanAnalysis.id);
  await metricsAdvancedService.generateFileComplexity(cleanAnalysis.id);
  const cleanMetrics = await prisma.metric.findFirst({ where: { analysisId: cleanAnalysis.id } });
  if (cleanMetrics) {
    await metricsAdvancedService.updateMetricsWithAdvanced(cleanAnalysis.id);
    await metricsAdvancedService.recordMetricsEvolution(cleanMetrics.id);
  }
  await metricsAdvancedService.recordQualityGateHistory(cleanProject.id, 'PASSED', 'All quality gates met');
