...
export class GitController {
  ...
  async cloneRepository(req: Request, res: Response) {
    try {
      ...
      const importData = await gitService.importRepository(gitUrl, branch);
  ...
  async getPullRequests(req: Request, res: Response) {
    try {
      ...
      const prs = await gitService.getPullRequests(owner, repo, state, limit);
  ...
  async analyzePullRequest(req: Request, res: Response) {
    try {
      ...
      const prs = await gitService.getPullRequests(owner, repo, 'all', 100);
   ...
   */
  async cleanupRepository(req: Request, res: Response) {
    try {
      res.json({
        success: true,
