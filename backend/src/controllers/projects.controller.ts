...
import { runAnalysis, runAnalysisBatch, detectLanguage, getProjectMetrics, getQualityGate } from '../services/analysis.service';
  ...
  try {
    ...
    const enriched = await Promise.all(
      projects.map(async project => {
        const qg = await getQualityGate(project.id);
...
export const getProject = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const qualityGate = await getQualityGate(id);
...
export const analyzeProject = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    runAnalysis(analysis.id, code, filename).catch(err => {
...
export const analyzeProjectBatch = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    runAnalysisBatch(analysis.id, files).catch(err => console.error('Batch analysis failed:', err));
...
export const getIssues = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const where: Record<string, unknown> = { analysisId: latestAnalysis.id };
    if (severity) where.severity = severity;
    if (type) where.type = type;
    if (file) where.file = { contains: file as string };
...
export const getQualityGateStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const qg = await getQualityGate(id);
    res.json(qg);
  } catch (error) {
 ...
 */
export const importLocalProjects = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as any).userId;
    ...
    const imported = await filesystemService.importLocalProjects(userId);
    res.json({
 ...
 */
export const discoverLocalProjects = async (req: Request, res: Response): Promise<void> => {
  try {
    const projects = await filesystemService.discoverProjects();
    res.json({
