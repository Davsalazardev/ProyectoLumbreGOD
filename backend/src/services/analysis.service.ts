...
import { Language, NormalizedIssue } from '../types';
import { analyzeJavaScript } from '../analyzers/javascript.analyzer';
import { analyzePython } from '../analyzers/python.analyzer';
import { analyzeJava } from '../analyzers/java.analyzer';
import { analyzeJSON } from '../analyzers/json.analyzer';
import { calculateMetrics, evaluateQualityGate } from './qualityGate.service';
import { notificationService } from './notification.service';
import { metricsService } from './metrics.service';
import { customRulesService } from './customRules.service';
import { hierarchyAnalysisService } from './hierarchyAnalysis.service';
...
export async function runAnalysis(
  ...
  try {
    ...
    switch (detectedLang) {
      case 'javascript':
      case 'typescript':
        issues = await analyzeJavaScript(code, filename);
        break;
      case 'python':
        issues = await analyzePython(code, filename);
        break;
      case 'java':
        issues = await analyzeJava(code, filename);
        break;
      default:
    ...
    const metrics = calculateMetrics(dedupedIssues);
    const loc = code.split("\n").length;
    const complexity = metricsService.calculateCyclomaticComplexity(code);
    const maintainability = metricsService.calculateMaintainabilityIndex(loc, complexity, metrics.bugs, metrics.codeSmells);
    const sqaleDebt = metricsService.calculateSqaleDebt(metrics.bugs, metrics.codeSmells, metrics.vulnerabilities, loc);
    ...
    if (project) {
      ...
      await notificationService.notifyAnalysisComplete(project.id, analysisId, metrics);

      // Notify on CRITICAL issues
      const criticalIssues = dedupedIssues.filter(i => i.severity === 'CRITICAL');
      for (const issue of criticalIssues) {
        await notificationService.notifyCriticalIssue(project.id, issue);
      }

      // Check quality gate
      const qualityGate = await getQualityGate(project.id);
      if (qualityGate.status !== 'PASSED') {
        await notificationService.notifyQualityGateFailed(project.id, qualityGate.conditions.map((c: any) => c.condition));
      }
    }
...
export async function getQualityGate(projectId: string) {
  const latestAnalysis = await prisma.analysis.findFirst({
    where: { projectId, status: 'COMPLETED' },
    orderBy: { startedAt: 'desc' },
    include: {
  ...
  return evaluateQualityGate(latestAnalysis.metrics, issueCounts);
}
 ...
 */
export async function runAnalysisWithoutHierarchy(
  analysisId: string,
  code: string,
  filename: string,
  language?: Language
  ...
  try {
    ...
    if (isSupported) {
      ...
      switch (detectedLang) {
        ...
        case 'json':
        case 'yaml':
          issues = await analyzeJSON(code, filename);
          break;
        default:
