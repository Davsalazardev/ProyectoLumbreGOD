...
export class MetricsService {
  ...
  calculateSecurityRating(vulnerabilityCount: number): string {
    ...
    if (vulnerabilityCount <= 5) return 'B';
    if (vulnerabilityCount <= 15) return 'C';
    if (vulnerabilityCount <= 50) return 'D';
    return 'E';
  }
   ...
   */
  async getProjectMetricsHistory(projectId: string, limit = 10) {
