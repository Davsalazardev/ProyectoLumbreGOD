...
function mapSeverity(eslintSeverity, ruleId) {
    ...
    if (ruleId && criticalRules.includes(ruleId))
        return 'CRITICAL';
    if (ruleId && majorRules.includes(ruleId))
        return 'MAJOR';
    if (eslintSeverity === 2)
        return 'MAJOR';
    if (eslintSeverity === 1)
...
function mapType(ruleId) {
    ...
    if (!ruleId)
        return 'CODE_SMELL';
    if (vulnRules.some(r => ruleId.includes(r)))
...
function analyzePatterns(code, filename) {
    const issues = [];
