...
function mapPylintSeverity(code) {
    if (code.startsWith('E') || code.startsWith('F'))
        return 'MAJOR';
    if (code.startsWith('W'))
        return 'MINOR';
    if (code.startsWith('C') || code.startsWith('R'))
        return 'INFO';
    return 'INFO';
}
function mapPylintType(code, message) {
    ...
    if (securityKeywords.some(kw => msgLower.includes(kw)))
        return 'VULNERABILITY';
    if (bugCodes.includes(code) || code.startsWith('E'))
        return 'BUG';
    return 'CODE_SMELL';
}
// Pattern-based Python analysis - 50+ RULES
function analyzePythonPatterns(code, filename) {
    const issues = [];
