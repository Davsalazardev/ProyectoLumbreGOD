...
const cicd_service_1 = require("../services/cicd.service");
...
router.post('/repositories/import', async (req, res) => {
    try {
        ...
        const importData = await git_service_1.gitService.importRepository(url, branch);
        ...
        res.status(500).json({ error: `Failed to import repository: ${error.message}` });
    }
});
/**
...
router.post('/repositories/:projectId/webhook', async (req, res) => {
    try {
        ...
        const webhook = await git_service_1.gitService.setupWebhook(owner, repo, webhookUrl);
        res.json({
            success: true,
            webhook,
...
router.post('/analysis/:projectId/performance', async (req, res) => {
    try {
        ...
        const memoryLeaks = performance_service_1.performanceService.analyzeMemoryLeaks(code, language);
        const concurrencyIssues = performance_service_1.performanceService.analyzeConcurrencyIssues(code, language);
        const cpuIntensive = performance_service_1.performanceService.analyzeCPUIntensive(code, language);
        const ioOperations = performance_service_1.performanceService.analyzeIOOperations(code, language);
        const allIssues = [...memoryLeaks, ...concurrencyIssues, ...cpuIntensive, ...ioOperations];
        const profile = performance_service_1.performanceService.generatePerformanceProfile(allIssues);
        res.json({
...
router.post('/analysis/:projectId/ml/predict-bugs', async (req, res) => {
    try {
        const { projectId } = req.params;
        const { code, language = 'javascript' } = req.body;
        const prediction = await ml_service_1.mlService.predictBugProbability(code, language);
        const anomalies = await ml_service_1.mlService.detectAnomalies(code, language);
        const refactoringPlan = await ml_service_1.mlService.generateRefactoringPlan(code, language);
        res.json({
...
router.post('/projects/:projectId/code-reviews', async (req, res) => {
    try {
        ...
        const review = await collaboration_service_1.collaborationService.createCodeReview(projectId, issueId, reviewerId, comments, codeSnippet);
...
router.post('/projects/:projectId/discussions', async (req, res) => {
    try {
        ...
        const discussion = await collaboration_service_1.collaborationService.createDiscussionThread(projectId, title, authorId, content);
...
router.get('/projects/:projectId/activity-feed', async (req, res) => {
    try {
        ...
        const activities = await collaboration_service_1.collaborationService.getTeamActivityFeed(projectId, parseInt(limit));
        res.json({ activities });
    }
    catch (error) {
...
router.post('/webhooks/github', async (req, res) => {
    try {
        ...
        if (event === 'workflow_run') {
            const result = await cicd_service_1.cicdService.processGitHubActionsWebhook(payload);
...
router.get('/search', async (req, res) => {
    try {
        ...
        const results = await search_service_1.searchService.fullTextSearch(q, projectId, filters);
...
router.post('/search/save', async (req, res) => {
    try {
        ...
        const savedSearch = await search_service_1.searchService.saveSearch(userId, name, query, filters);
...
router.post('/projects/:projectId/scan-dependencies', async (req, res) => {
    try {
        ...
        const scanResults = await search_service_1.dependencyService.scanDependencies(project.path, language);
...
router.post('/code-review/suggestions', async (req, res) => {
    try {
        const { code, language = 'javascript' } = req.body;
        const suggestions = await search_service_1.codeReviewService.generateReviewSuggestions(code, language);
        res.json({
            suggestions,
