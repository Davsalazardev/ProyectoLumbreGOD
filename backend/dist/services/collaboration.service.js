...
exports.collaborationService = {
     ...
     */
    async createCodeReview(projectId, issueId, reviewerId, comments, codeSnippet) {
     ...
     */
    generateReviewSuggestions(comments, codeSnippet) {
        const suggestions = [];
     ...
     */
    async addInlineComment(issueId, lineNumber, filePath, authorId, text) {
        return {
            issueId,
            lineNumber,
            filePath,
     ...
     */
    async createDiscussionThread(projectId, title, authorId, content) {
        return {
     ...
     */
    async replyToDiscussion(threadId, authorId, text, replyingToId) {
        return {
            id: `reply-${Date.now()}`,
            threadId,
            authorId,
     ...
     */
    async requestChanges(reviewId, reviewerId, requestedChanges) {
        return {
            reviewId,
            reviewerId,
     ...
     */
    calculateChangePriority(changes) {
        let criticalCount = 0;
     ...
     */
    async createTeamTask(projectId, title, description, assignedTo, priority, dueDate, relatedIssueIds) {
        return {
            id: `task-${Date.now()}`,
            projectId,
            title,
     ...
     */
    async getTeamActivityFeed(projectId, limit = 50) {
     ...
     */
    async getTeamMetrics(projectId, timeRange = 'month') {
        return {
            period: timeRange,
...
- [ ] New and existing unit tests passed locally with my changes`;
     ...
     */
    generateCodeReviewChecklist(language) {
