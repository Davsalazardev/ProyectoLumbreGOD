...
exports.hierarchyAnalysisService = exports.HierarchyAnalysisService = void 0;
...
class HierarchyAnalysisService {
     ...
     */
    async organizeIssuesByHierarchy(analysisId, issues) {
        try {
     ...
     */
    async getFolderHierarchy(analysisId) {
        try {
            ...
            for (const metric of folderMetrics) {
                ...
                const parts = metric.folderPath.split(path_1.default.sep).filter(p => p && p !== '.');
                let current = tree;
                for (let i = 0; i < parts.length; i++) {
                    const part = parts[i];
                    const fullPath = parts.slice(0, i + 1).join(path_1.default.sep);
            ...
            for (const fileMetric of fileMetrics) {
                ...
                const dirParts = dirPath.split(path_1.default.sep).filter(p => p && p !== '.');
                let current = tree;
                ...
                for (let i = 0; i < dirParts.length; i++) {
                    ...
                    const pathUpToHere = dirParts.slice(0, i + 1).join(path_1.default.sep);
                    let child = current.children?.find((c) => c.name === part && c.isFolder);
                    if (!child) {
                        child = {
                            name: part,
     ...
     */
    async getIssuesByFolder(analysisId, folderPath) {
        try {
     ...
     */
    async getIssuesByFile(analysisId, filePath) {
        try {
            const issues = await prisma.issue.findMany({
                where: {
                    analysisId,
     ...
     */
    async getMetricsTrend(projectId, limit = 20) {
        try {
...
exports.HierarchyAnalysisService = HierarchyAnalysisService;
exports.hierarchyAnalysisService = new HierarchyAnalysisService();
//# sourceMappingURL=hierarchyAnalysis.service.js.map