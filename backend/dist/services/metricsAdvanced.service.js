...
exports.metricsAdvancedService = void 0;
...
exports.metricsAdvancedService = {
    // Assign OWASP & CWE categories to issues
    async assignSecurityMappings(analysisId) {
        ...
        for (const issue of issues) {
            ...
            await prisma.issue.update({
                ...
                data: {
                    ...
                    hotspotRiskLevel: issue.severity === 'CRITICAL'
                        ? 'HIGH'
    ...
    async calculateCognitiveComplexity(analysisId) {
    ...
    async calculateTechnicalDebtRatio(analysisId) {
        const metrics = await prisma.metric.findUnique({ where: { analysisId } });
    ...
    async calculateIssueAge(analysisId) {
        const issues = await prisma.issue.findMany({
            where: { analysisId }
        });
        const now = new Date();
    ...
    async generateCodeDistribution(analysisId) {
        const metrics = await prisma.metric.findUnique({ where: { analysisId } });
        if (!metrics)
    ...
    async generateTestResults(analysisId) {
    ...
    async generateFileComplexity(analysisId) {
    ...
    async createSecurityHotspots(analysisId) {
        ...
        for (let i = 0; i < hotspots.length; i++) {
            ...
            await prisma.securityHotspotDetail.create({
                data: {
                    ...
                    probability: hotspot.severity === 'CRITICAL'
                        ? 'HIGH'
    ...
    async updateMetricsWithAdvanced(analysisId) {
        const metrics = await prisma.metric.findUnique({ where: { analysisId } });
        if (!metrics)
            return;
    ...
    async recordQualityGateHistory(projectId, status, reason) {
    ...
    async recordMetricsEvolution(metricId) {
...
//# sourceMappingURL=metricsAdvanced.service.js.map