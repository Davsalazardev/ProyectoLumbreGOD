...
class ReportService {
     ...
     */
    async generateComplianceReport(projectId, format = 'json') {
     ...
     */
    async generateSecurityReview(projectId) {
        const analyses = await prisma.analysis.findMany({
            where: { projectId, status: 'COMPLETED' },
            orderBy: { startedAt: 'desc' },
            take: 5,
    ...
    analyzeTrend(analyses) {
        ...
        const latest = analyses[0].issues.filter((i) => i.type === 'VULNERABILITY').length;
        const previous = analyses[1].issues.filter((i) => i.type === 'VULNERABILITY').length;
        if (latest < previous)
            return 'IMPROVING';
        if (latest > previous)
            return 'DEGRADING';
